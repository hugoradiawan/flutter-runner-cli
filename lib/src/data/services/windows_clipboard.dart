...
bool windowsSetClipboardUtf16(String text) {
  ...
  try {
    ...
    final globalLock = kernel32.lookupFunction<_GlobalLockC, _GlobalLockDart>(
      'GlobalLock',
    );
    final globalUnlock = kernel32
        .lookupFunction<_GlobalUnlockC, _GlobalUnlockDart>('GlobalUnlock');
    final globalFree = kernel32.lookupFunction<_GlobalFreeC, _GlobalFreeDart>(
      'GlobalFree',
    );
    final openClipboard = user32
        .lookupFunction<_OpenClipboardC, _OpenClipboardDart>('OpenClipboard');
    final emptyClipboard = user32
        .lookupFunction<_EmptyClipboardC, _EmptyClipboardDart>(
          'EmptyClipboard',
        );
    final setClipboardData = user32
        .lookupFunction<_SetClipboardDataC, _SetClipboardDataDart>(
          'SetClipboardData',
        );
    final closeClipboard = user32
        .lookupFunction<_CloseClipboardC, _CloseClipboardDart>(
          'CloseClipboard',
        );
